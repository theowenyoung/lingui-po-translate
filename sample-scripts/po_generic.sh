...
lingui-ai-translate "${COMMON_ARGS[@]}" --targetFile=$BASE_DIR/es.po --targetLng=es
lingui-ai-translate "${COMMON_ARGS[@]}" --targetFile=$BASE_DIR/de.po --targetLng=de

# Convert a YAML to PO (just for the sake of test-coverage)
lingui-ai-translate --srcFile="$BASE_DIR/nested-fruits.yml" --srcFormat=yaml --srcLng=x --targetFile=$BASE_DIR/nested-fruits.po --targetFormat=po --targetLng=x --service=sync-without-translate
