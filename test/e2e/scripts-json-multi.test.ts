...
test("missing OpenAI key", async () => {
  const output =
    await runCommandExpectFailure(`cd sample-scripts && lingui-ai-translate --srcFile=json-simple/en.json --srcLng=English --srcFormat=nested-json --targetFile=json-simple/es.json --targetLng=German --targetFormat=nested-json --service=openai
  `);
...
test("invalid OpenAI key", async () => {
  const output =
    await runCommandExpectFailure(`cd sample-scripts && lingui-ai-translate --srcFile=json-simple/en.json --srcLng=English --srcFormat=nested-json --targetFile=json-simple/es.json --targetLng=German --targetFormat=nested-json --service=openai --serviceConfig=garbageapikey
  `);
  expect(output).toContain(
...
test("missing OpenAI key (typechat)", async () => {
  const output = await runCommandExpectFailure(
    `cd sample-scripts && lingui-ai-translate --srcFile=json-simple/en.json --srcLng=English --srcFormat=nested-json --targetFile=json-simple/es.json --targetLng=German --targetFormat=nested-json --service=typechat
  `,
    undefined,
